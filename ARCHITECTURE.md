...
flowchart LR
  ...
  subgraph Dev["Developer / Local"]
    ...
    BackendSrc["backend, src, services"]
    TrainScript["src/models/train_model.py"]
    Tests["tests/"]
    AIDE -->|Edit code / notebooks| BackendSrc
    AIDE -->|Run training| TrainScript
    LocalEnv -->|Run API| BackendSrc
    LocalEnv -->|Run tests| Tests
  end
  ...
  subgraph ModelRegistry["Model Registry / Storage"]
    ...
    RegisteredModel["Registered Model: delivery_time_pred_model_pipe"]
    MLflow -->|register model| RegisteredModel
